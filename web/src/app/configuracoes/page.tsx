...
export default function Configuracoes() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col justify-between text-gray-650 dark:text-zinc-400 font-sans antialiased pt-16">
      <Navbar />
      
      <main className="flex-1 max-w-2xl mx-auto px-6 py-12 md:py-20 w-full space-y-8">
        {/* Title */}
        <div className="space-y-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/50">
        ...
        {/* Settings Card */}
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-8 rounded-lg space-y-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 flex items-center gap-2 border-b border-gray-200 dark:border-zinc-800 pb-3 font-mono">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-gray-500 dark:text-zinc-450 text-xs leading-relaxed max-w-sm">
            ...
            <button
              ...
              className="p-1 rounded-full text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors cursor-pointer bg-transparent border-0"
              title={blogAdminMode ? "Desativar Modo Admin" : "Ativar Modo Admin"}
            >
              {blogAdminMode ? (
                <ToggleRight size={40} className="text-indigo-600 dark:text-indigo-550" />
              ) : (
                <ToggleLeft size={40} className="text-gray-400 dark:text-zinc-600" />
              )}
            </button>
          </div>

          {blogAdminMode && (
            <div className="bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/50 p-4 rounded-lg text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-2.5">
